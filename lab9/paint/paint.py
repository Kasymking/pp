...
# TOOL SETTINGS
thicknesses = [15, 5, 5, 5, 5, 5, 5, 5]  # [Eraser, Brush, Line, Square, Circle, Diamond, R_triangle, E_Triangle]
mod = 1  # Current tool (default is Brush)
LMBpressed = False

# TOOLBAR (tools and colors)
tools = ["Eraser", "Brush", "Line", "Square","Circle", "Diamond", "R_Triangle", "E_Triangle"] 
...
# Calculate square coordinates from two points
def calculate_square(x1, y1, x2, y2):
    ...
    return pygame.Rect(top_left_x, top_left_y, side_length, side_length)
...
def draw_figure(figure_index, surface):
    ...
    elif figure_index == 3:  # square
        pygame.draw.rect(surface, curr_color, calculate_square(prev_x, prev_y, curr_x, curr_y), thicknesses[3])
    elif figure_index == 4:
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION and LMBpressed:
            ...
            elif mod in [2, 3, 4, 5, 6, 7]:  # Line and square are temporarily drawn on the screen
                curr_x, curr_y = event.pos
        ...
        if event.type == pygame.KEYDOWN: 
            ...
            if event.key == pygame.K_r: mod = 3  # Square
